...
import { APIPromise } from '@valtown/sdk/core/api-promise';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['VAL_TOWN_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: ValTown) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    const client = new ValTown({ logger: logger, bearerToken: 'My Bearer Token' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new ValTown({ logger: logger, bearerToken: 'My Bearer Token' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'VAL_TOWN_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new ValTown({ baseURL: 'http://localhost:5000/custom/path/', bearerToken: 'My Bearer Token' });
    ...
    afterEach(() => {
      process.env['VAL_TOWN_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new ValTown({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['VAL_TOWN_BASE_URL'] = 'https://example.com/from_env';
      const client = new ValTown({ bearerToken: 'My Bearer Token' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new ValTown({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new ValTown({ bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['VAL_TOWN_API_KEY'] = 'My Bearer Token';
    const client = new ValTown();
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new ValTown({ bearerToken: 'My Bearer Token', fetch: testFetch });
